...
serve_read(envid_t envid, union Fsipc *ipc)
{
	...
	// envid is regular env who request read
	// find the file to read
	if ((r = openfile_lookup(envid, req->req_fileid, &opfile)) < 0) {
		return r;
	}
	if ((r = file_read(opfile->o_file, ret->ret_buf, req->req_n, opfile->o_fd->fd_offset)) < 0) {
		return r;
	}
	assert(r >= 0);
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
	...
	int r;
	struct OpenFile *opfile;
	// envid is regular env who request write
	// find the file to write
	if ((r = openfile_lookup(envid, req->req_fileid, &opfile)) < 0) {
		return r;
	}
	if ((r = file_write(opfile->o_file, req->req_buf, req->req_n, opfile->o_fd->fd_offset)) < 0) {
		return r;
	}
	assert(r >= 0);
	opfile->o_fd->fd_offset += r;
